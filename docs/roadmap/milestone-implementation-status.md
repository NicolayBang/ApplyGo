...
| **M5 - Packet MVP** | Draft CV bullets, cover note, screening answers, store packet. | Implemented for the reusable document/answer model and packet read-model API. Migrations `0012`–`0014` provide `documents`, immutable `document_versions`, append-only `application_documents`, mutable `answer_library`, and immutable `application_answers`; the dashboard can preview packet material from existing evidence, prepare deterministic cover-note/packet text, copy/download reviewer material, persist packet review decisions, expose latest packet review, and append `application_packet.reviewed` audit evidence. | Full packet text is not stored by default as an immutable versioned artifact, and end-to-end packet authoring UX remains a future product decision. | `docs/contracts/m5-packet-document-answer-contract.md`; `docs/contracts/m2-application-packet-contract.md`; `docs/contracts/m2-packet-persistence-contract.md`; `docs/decisions/ADR-0002-canonical-data-model.md` (Proposed governing direction); `docs/architecture/database-implementation-roadmap.md`; `docs/roadmap/m2-scope-and-acceptance.md`; `docs/codebase/c4-code-frontend.md`. | ADR-0002 remains Proposed as governance status; no schema migration approval is pending for the implemented M5 baseline. | Treat the M5 schema/API cutover as complete; maintain regression coverage and define any packet-authoring UX separately. |
